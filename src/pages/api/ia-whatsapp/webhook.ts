// Adaptado para usar a API do Next.js na Vercel
 ...
 * Adaptado para o formato de API da Vercel
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    if (configError || !config) {
      ...
      return res.status(500).json({ status: 'error', message: 'Configuração de IA não encontrada' });
    }
    ...
    if (messageError) {
      ...
      return res.status(500).json({ status: 'error', message: 'Erro ao processar mensagem' });
    }
    ...
    if (estError || !establishment) {
      ...
      return res.status(500).json({ status: 'error', message: 'Estabelecimento não encontrado' });
    }
    ...
    const formattedHistory = (conversationHistory || [])
      ...
      .map(msg => {
        if (msg.direction === 'incoming') {
          return { role: 'user' as 'user' | 'assistant' | 'system', content: msg.content };
        } else {
          return { role: 'assistant' as 'user' | 'assistant' | 'system', content: msg.response || '' };
        }
      });
    ...
    try {
      ...
      aiResponse = await openaiService.processMessage(
        content,
        JSON.stringify(context), // Convertemos o contexto para string
        formattedHistory
      );
    } catch (aiError) {
